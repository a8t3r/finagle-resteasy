...
public class TestNettyHeaderWrapper {
    ...
    public void testGetFirst() throws Exception {
        response.addHeader("k", "v1");
        response.addHeader("k", "v2");
        assertMapping("k", "v1", "v2");
        // weird semantics: "getFirst" actually means "get the most recently
        // added", which in this case is ...
        assertEquals("v2", response.getHeader("k"));
        assertEquals("v2", wrapper.getFirst("k"));
    }
    ...
    public void testKeySet() throws Exception {
        ...
        for (String id : ids) {
            response.addHeader(id, "v-" + id);
        }
        assertEquals("response has wrong keys", ids, this.response.getHeaderNames());
